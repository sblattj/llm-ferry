...
**MTP speculative decoding is OFF on `local-orch` (crash, fixed 2026-08-25):** with the KV governor (`--kv-bits 4`) *and* an MTP draft model, the draft-verify branch of mlx-vlm's qwen3_5 attention crashed on **every cache-hit request** (turn 2+ of any conversation): the quantized cache returns keys as a tuple of `(packed, scales, biases)` arrays, and `prefix_len = keys.shape[-2]` raised `AttributeError: 'tuple' object has no attribute 'shape'` → the client saw `APIConnectionError` / 500. Deterministic repro: the same request twice → 200 then 500. Fix: `LOCAL_DRAFT_ORCH` now defaults to empty (no `--draft-model`); set it to re-enable at your own risk, ideally together with clearing `LOCAL_ORCH_KV_BITS`. The `local-sub` lane never had a draft model and is unaffected.

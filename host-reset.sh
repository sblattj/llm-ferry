...
# _seed_master_key — generate LITELLM_MASTER_KEY when the live route config
# gates the front door on it and no key is reachable. v1.22.0's template sets
# general_settings.master_key: os.environ/LITELLM_MASTER_KEY, and the validator
# below refuses to restart while a referenced var is unset — so the first reset
# after adopting the template would otherwise dead-end by design. Generate once,
# append to secrets.env (0600), and never print the value: it exists so the
# operator can hand it to clients, not so this script can echo it.
# A key already in the shell or secrets.env always wins; the function is a
# no-op then, and a no-op when the config does not reference the var at all
# (keyless LAN installs are untouched).
_seed_master_key() {
  [[ -f "$ROUTE_CONFIG" ]] || return 0
  grep -q "os.environ/LITELLM_MASTER_KEY" "$ROUTE_CONFIG" 2>/dev/null || return 0
  [[ -n "${LITELLM_MASTER_KEY:-}" ]] && return 0
  if [[ -f "$SECRETS" ]] && grep -qE '^[[:space:]]*(export[[:space:]]+)?LITELLM_MASTER_KEY=' "$SECRETS" 2>/dev/null; then
    return 0
  fi
  local key
  key="$(python3 -c "import secrets; print('sk-'+secrets.token_urlsafe(24))")"
  [[ -n "$key" ]] || die "could not generate LITELLM_MASTER_KEY (python3 unavailable?)."
  if [[ ! -f "$SECRETS" ]]; then
  ...
  {
    echo ""
    echo "# Generated by host-reset.sh $(date -u +%Y-%m-%dT%H:%M:%SZ): front-door master_key."
    echo "export LITELLM_MASTER_KEY=$key"
  } >> "$SECRETS"
  # secrets.env holds every provider key; a seeded write must never leave it
  # looser than the documented 0600 (an earlier 0644 would otherwise survive).
  chmod 600 "$SECRETS"
  unset key
  ok "generated a LITELLM_MASTER_KEY for the front door -> $SECRETS (0600)."
  ok "the key itself is never printed; read it from $SECRETS to hand it to clients."
}
...
# Seed before validating: the validator fails closed on a referenced-but-unset
# var, so a fresh master_key adoption would never get past this point without
# a key to find.
_seed_master_key
...
# /health/liveliness, not /v1/models: this loop only asks "is the process
# serving HTTP", and under general_settings.master_key (v1.22.0) the catalogue
# route answers 401 to an unauthenticated probe. liveliness is one of litellm's
# public routes, so the probe needs no key and behaves identically with or
# without one. The catalogue itself is checked with auth in the verifier below.
waited=0
while ! curl -fsS -m 3 "http://127.0.0.1:$PORT/health/liveliness" >/dev/null 2>&1; do
...
# dumb KEY=VALUE read the validator uses, never sourcing the file) — and present
# it when we have one. A 401 with no key at hand degrades the listing to a note
# instead of crashing the run; a key that is REJECTED is a real problem, because
# it means the running proxy was started under a different key than this reset
# would serve.
def load_master_key():
...
try:
    ...
    if master_key:
        key_rejected = True
        print("    PROBLEM: /v1/models rejected LITELLM_MASTER_KEY — the running proxy "
              "was started under a different key than the shell/secrets.env one.",
              file=sys.stderr)
    else:
        print("    note: the front door required auth on /v1/models (the proxy runs "
              "behind master_key) but LITELLM_MASTER_KEY is in neither the shell nor "
              f"{secrets_path} — the lane listing below is skipped. Add the key to "
              "secrets.env to restore this check.")
if listing_known:
...
for c in CONFIGS:
    ...
    if bad and aliases_known and listing_known:
        print(f"    {os.path.basename(p)}: names {', '.join(bad)} — NOT served", file=sys.stderr)
        failed = True
    elif bad:
        why = "auth blocked the listing" if not listing_known else "see the note above"
        print(f"    {os.path.basename(p)}: {', '.join(bad)} unconfirmed ({why})")
    else:

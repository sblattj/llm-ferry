...
**`POST /v1/ferry/fleet`** with `{"fleet": "international"}` — sets the caller's own sticky selection, identity resolved exactly as in section 4. Bearer required when a master key is configured; this is the first LAN-reachable mutation, and it can only ever change the caller's own entry. Identity is whatever the request declares (section 4), so the host — the dash on loopback, or `ferry fleet` with `X-Ferry-Client` — moves another client by speaking as it; the guarantee is that a request cannot touch an entry it did not name, not that names are authenticated (section 10). With `{"fleet": "international", "default": true}` it sets the host-wide default and is loopback-only as seen by the ASGI peer address, like every other mutation today. Under the README's remote-access recipe (`tailscale serve … http://127.0.0.1:8090`) every tailnet peer arrives as loopback, so there the bearer is the gate, not the address. `{"fleet": null}` clears the caller's entry so they follow the default again. The response is the same document as GET, after the write. The write is atomic (temp file + rename) so a worker never reads a half-written file.
